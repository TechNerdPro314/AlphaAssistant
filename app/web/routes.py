...
def login():
    ...
    if request.method == 'POST':
        ...
        try:
            # Выступаем клиентом для нашего же API
            response = requests.post(f"{API_BASE_URL}/auth/login", json={"email": email, "password": password})
            if response.status_code == 200:
                user = User.query.filter_by(email=email).first()
                login_user(user, remember=True) # Логиним пользователя через Flask-Login
                
                # Сохраняем JWT токен в сессии браузера для использования в JS
...
def logout():
    logout_user()
    session.pop('jwt_token', None) # Удаляем токен из сессии
...
@bp.route('/chat')
@login_required # Защищаем эту страницу от анонимных пользователей
def chat():
    # Передаем JWT токен в шаблон, чтобы JS мог его использовать
