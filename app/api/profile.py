...
business_profile_model = api.model(
    "BusinessProfile",
    {
        "industry": fields.String(required=True, description="Отрасль бизнеса"),
        "company_size": fields.String(
            required=True, description="Размер компании (например, 1-10 сотрудников)"
        ),
        "goals": fields.String(
            required=True, description="Основные цели и задачи бизнеса"
        ),
    },
)

business_profile_response_model = api.inherit(
...
class ProfileResource(Resource):
    ...
    @api.doc(security="jwt")
    @jwt_required()
    @api.expect(business_profile_model, validate=True)
    @api.marshal_with(business_profile_response_model)
        ...
        api.model(
            "LinkTelegramModel",
            {
                "telegram_id": fields.String(
                    required=True, description="Уникальный ID пользователя в Telegram"
                )
            },
        )
    )
